...
String.prototype.after = function (parMatch, blnCaseSensitive) {
    ...
    if (this.length == 0 || strMatch.length == 0) return retVal;

    blnCaseSensitive = blnCaseSensitive | (String(blnCaseSensitive) == 'undefined'); //default is true
    var lngFind = 0;
    if (blnCaseSensitive) lngFind = this.indexOf(strMatch);
    else
    lngFind = this.toUpperCase().indexOf(strMatch.toUpperCase());

    if (lngFind >= 0) {
        if (lngFind <= (this.length - strMatch.length)) retVal = this.substr(lngFind + strMatch.length);
    }
...
String.prototype.afterRev = function (parMatch, blnCaseSensitive) {
    ...
    var lngFind = 0;
    if (blnCaseSensitive) lngFind = this.lastIndexOf(strMatch);
    else
...
String.prototype.before = function (parMatch, blnCaseSensitive) {
    ...
    if (lngFind > 0) retVal = this.substr(0, lngFind);
...
String.prototype.contains = function (parMatch, blnCaseSensitive) {
    ...
    if (strMatch.length == 0) return true; //per XPath spec
    blnCaseSensitive = blnCaseSensitive | (String(blnCaseSensitive) == 'undefined');

    if (blnCaseSensitive) retVal = (this.indexOf(strMatch) > -1);
    else
...
String.prototype.startsWith = function (parMatch, blnCaseSensitive) {
    ...
    if (strMatch.length == 0) return true; //per XPath spec
    if (strMatch.length > this.length) return false;

    blnCaseSensitive = blnCaseSensitive | (String(blnCaseSensitive) == 'undefined');

    if (blnCaseSensitive) retVal = (this.indexOf(strMatch) == 0);
    else
...
String.prototype.translate = function (parFrom, parTo, blnCaseSensitive) {
    ...
    for (var i = 0; i < this.length; i++) {
        ...
        if (blnCaseSensitive) lngPos = strFrom.indexOf(strBuffer);
        else
        lngPos = (strFrom.toUpperCase()).indexOf((strBuffer.toUpperCase()));

        if (lngPos > -1) strCopy = strCopy + strTo.charAt(lngPos);
        else
